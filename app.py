...
def get_test_success():
    ...
    return df[df.result == 0].count().unique()[0]
                ...
                children= [
                    html.Div(
                            ...
                            children=[
                                    ...
                                    html.Div(className="cluster-label", children='Set Cluster Number'),
                                    daq.Knob(
                    ...
                    html.Div(
                            ...
                            children=[
                                    ...
                                    dcc.Dropdown(
                                        ...
                                        multi=True,
                                        placeholder="Select features",
                                        value=['time', 'line_numbers', 'per_test_iterations']
...
def update_progress_gauge(*args):
...
def timeseries_chart(selected_dropdown_value, *args): #btn_0_click, btn_1_click, btn_2_click, btn_3_click, btn_4_click):
    ...
    #commit_value = check_commit_memory()
    ...
    for stock in selected_dropdown_value:
        trace.append(go.Scatter(x=df_sub[df_sub['test_name'] == stock].index.values // 65,
...
def input_clustering(clustering_type, feature_name, check_list_type, cluster_value, *args):
    ...
    if '2D' in check_list_type:
             fig = px.scatter(df, x= feature_name[0],y= feature_name[1], color=df['Cluster'])
    elif '3D' in check_list_type:
            fig= px.scatter_3d(df, x = feature_name[0], y=feature_name[1], z=feature_name[2],
              color='Cluster', opacity = 0.8, size='size', size_max=70)
    #fig = dict()
...
def test_clustering(clustering_type, feature_name, check_list_type, cluster_value, *args):
    ...
    if len (feature_name) < 2:
        feature_name = ['time', 'per_test_iterations','line_numbers' ]
    ...
    if '2D' in check_list_type:
            ...
            fig= px.scatter_3d(df, x = feature_name[0], y=feature_name[1], z=feature_name[2],
              color='Cluster', opacity = 0.8, size_max=70)
    #fig = dict()
    fig.update_layout(colorway=['rgb(85, 255, 241)'],
                  template='plotly_dark',
                  paper_bgcolor='rgba(0, 0, 0, 0)',
